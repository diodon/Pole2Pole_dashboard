...
siteDF$eventDate = ymd(paste0(siteDF$yyear, siteDF$mmonth, siteDF$dday))
...
```{r savefiles}
fileNameSite = paste(siteDF$country[1], gsub(" ", "", siteDF$locality[1]), "dashboard_siteDF.csv", sep="_")
fileNameOccurrence = paste(siteDF$country[1], gsub(" ", "", siteDF$locality[1]), "dashboard_occurrence.csv", sep="_")

readr::write_csv(path = file.path(baseDataDir, "DataAnalysisFiles", fileNameSite), siteDF)
readr::write_csv(path = file.path(baseDataDir,"DataAnalysisFiles", fileNameOccurrence), Occurrence)
